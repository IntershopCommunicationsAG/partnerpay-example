...
public class CancelImpl implements Cancel
{
    ...
    public Result<CancelResult> cancel(PaymentContext context, Payable payable)
    {
        ...
        Logger.debug(this, "Cancellation triggered for payment transaction {}", context.getPaymentTransaction().getId());
    ...
    {
        Optional<Date> optCreated = pt.getPaymentHistoryEntries().stream().map(ph -> ph.getEventTime()).min(Comparator.naturalOrder());
        
        if (!optCreated.isPresent())
        {
            Logger.error(this, "There is no payment history for this payment transaction {}. "
                            + "This should not normally happen. "
                            + "We will consider that a cancel attempt can be made though.", pt.getId());
            return true;
        }
        else
        {
            ...
            Logger.debug(this, "The payment transaction is {} hours old. Max allowed timeout is {}. Can be cancelled {}", 
